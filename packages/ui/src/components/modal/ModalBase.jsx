    ...
    <Dialog open={isOpen} onClose={dismiss} className="relative z-50">
      ...
      <div className="fixed inset-0 z-50 w-screen overflow-y-auto overscroll-contain">
        <div className="flex min-h-full items-center justify-center p-4 text-left">
          <TransitionChild afterLeave={clear}>
            <DialogPanel
              className={cn(
                'w-full max-w-md origin-center transform overflow-hidden rounded-surface border border-border bg-background p-5 text-text shadow-overlay transition-[opacity,translate,scale] data-closed:translate-y-2 data-closed:scale-95 data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-150 data-leave:ease-in motion-reduce:data-closed:translate-y-0 motion-reduce:data-closed:scale-100 motion-reduce:data-enter:duration-150 motion-reduce:data-leave:duration-100',
                panelClassName,
              )}
            >
              {children}
